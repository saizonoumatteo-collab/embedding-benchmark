...
function renderRetrievalTab(models) {
  ...
  const rC=new Chart(rc,{type:'bar',data:{labels:good.map(s),datasets:[
    ...
    {label:'Top-1 Acc',data:good.map(m=>state.results[m].retrieval.top1_accuracy), backgroundColor:'#bc8cff88',borderColor:'#bc8cff',borderWidth:2,borderRadius:4},
  ...
  if(hasHard) {
    ...
    const hC=new Chart(hc,{type:'bar',data:{labels:good.map(s),datasets:[
      {label:'Rejet Hard Neg', data:good.map(m=>state.results[m].retrieval.hard_negative_rejection), backgroundColor:'#f8514988',borderColor:'#f85149',borderWidth:2,borderRadius:4},
      {label:'Rejet Contradiction',data:good.map(m=>state.results[m].retrieval.contradiction_rejection), backgroundColor:'#ff7b7288',borderColor:'#ff7b72',borderWidth:2,borderRadius:4},
      {label:'Rejet Distracteur',  data:good.map(m=>state.results[m].retrieval.distractor_rejection),   backgroundColor:'#e3b34188',borderColor:'#e3b341',borderWidth:2,borderRadius:4},
      {label:'NDCG@5 (hard only)', data:good.map(m=>state.results[m].retrieval.ndcg_at_5_hard_only),   backgroundColor:'#d2992288',borderColor:'#d29922',borderWidth:2,borderRadius:4},
    ]},options:{...CD(),scales:{...CD().scales,y:{...CD().scales.y,beginAtZero:true,max:1}}}});
    state.charts.push(hC);
  }
  ...
  if(ret0.ndcg_by_type && Object.keys(ret0.ndcg_by_type).length>1) {
    ...
    const tC=new Chart(tc,{type:'bar',
      data:{
        labels: types.map(t=>((QUERY_TYPE_META[t] && QUERY_TYPE_META[t].icon)||'')+'  '+((QUERY_TYPE_META[t] && QUERY_TYPE_META[t].label)||t)),
        datasets:[{
          label:'NDCG@5',
          data:types.map(t=>ret0.ndcg_by_type[t]),
          backgroundColor:types.map(t=>((QUERY_TYPE_META[t] && QUERY_TYPE_META[t].color)||'#58a6ff')+'88'),
          borderColor:types.map(t=>(QUERY_TYPE_META[t] && QUERY_TYPE_META[t].color)||'#58a6ff'),
  ...
  if((pq && pq.length)) {
    ...
    barChart(qc, pq.map(q=>((QUERY_TYPE_META[q.query_type] && QUERY_TYPE_META[q.query_type].icon)||'')+'  '+q.query.slice(0,32)),
      [{label:'NDCG@5',
